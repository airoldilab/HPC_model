...
if(use.true.params){
  ...
  current.param.list$xi.param.vecs <- current.param.list$xi.param.vecs[names(doc.length.vec),]

  ## # Perturb initial starting values
  ## #current.param.list$nu <- 6
  ## #current.param.list$sigma2 <- 1
  ## current.param.list$lambda2 <- 6
  ## current.param.list$eta.vec <- true.param.list$eta.vec + rnorm(n=length(true.param.list$eta.vec))
  ## current.param.list$psi <- true.param.list$psi + rnorm(1,sd=0.5)
  ## current.param.list$gamma <- 2
}
...
if(!use.true.params){
  ...
  ## current.param.list[["eta.vec"]] <- true.param.list$eta.vec
  ## current.param.list$mu.param.vecs <- true.param.list$mu.param.vecs
  ## current.param.list$mu.corpus.vec <- true.param.list$mu.corpus.vec
  ## current.param.list$tau2.param.vecs <- true.param.list$tau2.param.vecs
}
