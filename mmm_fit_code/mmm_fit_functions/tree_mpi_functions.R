...
tree.slave.fn <- function(current.param.list,doc.length.vec,
  ...
  while(done != 1){
    ...
    if(tag == 0){
      ...
      cat(sprintft("Worker %s refreshing likelihood hessian for word jobs\n",
                  worker.id))
      ...
      for(word.id in word.ids){
        ...
        out.optim.tree <- optim.tree(job.id=word.id,
        ...
        par <- c(out.optim.tree$mu.f,out.optim.tree$mu.0.f)
        mu.f.new <- out.optim.tree$mu.f
        mu.0.f.new <- out.optim.tree$mu.0.f
        tree.data.list <- out.optim.tree$tree.data.list

        ## # Get new tau2 draw conditional on mu draw
        ## tree.data.list$tau2f.vec <- tau2.draw(job.id=word.id,
        ##                                       mu.f=mu.f.new,mu.0.f=mu.0.f.new,
        ##                                       current.param.list=current.param.list)

        # Get hessian of likelihood at mode
        hessian.like.list[[word.id]] <- get.hessian.like(par=par,
    ...
    if(any(tag == 0,tag == 1)){ 
      ...
      for(word.id in word.ids){
        ...
        # Get new mu draw
        hmc.draws <- hmc.tree(job.id=word.id,ndraws=10,
                              step.size=0.2,nsteps=5,
                              current.param.list=current.param.list,
                              doc.length.vec=doc.length.vec,
                              doc.topic.list=doc.topic.list,
                              feature.count.list=feature.count.list,
                              topic.address.book=topic.address.book,
                              last.draw=TRUE,
                              hessian.like=hessian.like.list[[word.id]],
